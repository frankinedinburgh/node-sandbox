	...
	(function (){
        ...
        /* Buffer Utitlities */
			...
			MP4Tag.parse = function (handle, callback){
				...
				var containers = [
					"moof", "mfhd", "traf", "tfhd", "trun", "uuid", "mdat"
				];
          ...
          /* Parsing */
						...
						if (err) {
							...
							if (buffer.byteLength < 8) {
								...
								if (containers.indexOf(name) !== -1) {
									...
									console.log('Found box of type ' + name + ' and size ' + length);

									if (/mdat/.test(name)) {



                                        var fpp = DataView.decodeUtf8(dv.buffer);
				...
				readAtoms(0, function (err, atoms){
					if (!err) {
						...
						var ilst = findAtom(atoms, [ 'moof', 'mfhd', 'traf', 'tfhd', 'trun', 'uuid', 'mdat' ]);


						if (typeof ilst === 'undefined' || typeof ilst === null) {
							console.log(atoms)
							debugger;
						}
...
(function (){
        ...
        /*
         Pass the File instance to MP4.js
